...
export default function Login() {
  ...
  const ErrorBanner = () => error && (
    <div className="mb-4 p-3 rounded-lg text-sm" style={{ background: 'var(--color-danger-light)', border: '1px solid var(--color-danger)', color: 'var(--color-danger)' }}>
      {error}
    </div>
  );

  const MessageBanner = () => message && (
    <div className="mb-4 p-3 rounded-lg text-sm" style={{ background: 'var(--color-success-light)', border: '1px solid var(--color-success)', color: 'var(--color-success-hover)' }}>
      {message}
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-10" style={{ background: 'linear-gradient(135deg, var(--color-primary-light) 0%, var(--color-accent-light) 100%)' }}>
      <div className="card w-full max-w-md relative overflow-hidden transition-all duration-300">
